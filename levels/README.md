...
Only Level 1 plays the full tutorial. Later levels begin paused. Puzzle rules are conveyed by labels on the map and the objects' behavior; the toolbox contains controls and optional neural diagnostics, not solution hints. The toolbox chooser can replay any level; after winning, Next level or N advances. R resets the current level and neural state. F9 restores the level and mechanics captured by F5 along with its brain checkpoint.
