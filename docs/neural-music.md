...
volume is 35%. The score uses one mono mix, duplicated identically into the left
and right channels. An unavailable output leaves the game playable and offers **Retry
...
allocation, file access, neural work, or locking. The mono mix is bounded to
`[-0.5, 0.5]` at maximum volume and sent equally to both stereo channels. The
device requests two output channels, with Rodio's supported-format fallback.
...
After a report of left-only playback, the three voices now feed one mono mix,
duplicated identically to both stereo channels, and the output requests two
channels before supported-format fallback. The former gentle panning did not
explain a completely silent right channel; physical device routing has not been
independently diagnosed.

All 12 music tests and the desktop build pass. A regression renders through the
real Rodio mixer at 44.1 and 48 kHz and verifies nonzero, sample-identical left and
right output. `runs/neural-music-mono-preview.wav` is a newly rendered 17-second,
48 kHz, single-channel audition file. Reopen the worktree app to use the update.
